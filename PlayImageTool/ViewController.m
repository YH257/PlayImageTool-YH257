...
- (void)viewDidLoad {
    ...
    rightBtn.frame = CGRectMake(15, 7, 40, 30);
    [rightBtn setTitle:@"设置" forState:UIControlStateNormal];
    //    [rightBtn setImage: forState:UIControlStateNormal];
    [rightBtn setTitleColor:UIColor.redColor forState:UIControlStateNormal];
    [rightBtn addTarget:self action:@selector(pushSetVc) forControlEvents:UIControlEventTouchUpInside];
    [self.navigationController.navigationBar addSubview:rightBtn];
    

    UIButton *leftBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    leftBtn.frame = CGRectMake(self.view.frame.size.width - 55, 7, 40, 30);
    [leftBtn setTitle:@"添加" forState:UIControlStateNormal];
    //    [rightBtn setImage: forState:UIControlStateNormal];
    [leftBtn setTitleColor:UIColor.redColor forState:UIControlStateNormal];
    [leftBtn addTarget:self action:@selector(pushAddVc) forControlEvents:UIControlEventTouchUpInside];
    [self.navigationController.navigationBar addSubview:leftBtn];
    
    
    self.title = @"动物图";
    self.view.backgroundColor = fViewCtrlBgc;
    [self initData];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    vc.imgData = cell.imageView.image;
